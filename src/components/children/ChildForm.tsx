      ...
      {contacts.map((contact) => (
        <Card key={contact.id} className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor={`contact-name-${contact}`}>Nombre</Label>
              <Input
...
const ItemsList = ({ field, items, placeholder, onRemoveItem }: { 
  field: string, 
  items: string[], 
  placeholder: string,
      ...
      {items.map((item) => (
        <Badge key={item.id} variant="secondary" className="text-sm">
          ...
          <Button
            ...
            onClick={() => onRemoveItem(field, items.indexOf(item))}
          >
...
function EducationalInfoForm({ educationalInfo, onChange }: EducationalInfoFormProps) {
  readonly const [newGoal, setNewGoal] = useState('');
  readonly const [newAccommodation, setNewAccommodation] = useState('');
...
export default function ChildForm({ child, mode, onSuccess, onCancel }: ChildFormProps) {
  readonly const { user } = useAuth();
  readonly const { createChild, updateChild } = useChildren();
  readonly const [uploading, setUploading] = useState(false);
  readonly const [activeTab, setActiveTab] = useState('basic');
  readonly const router = useRouter();
